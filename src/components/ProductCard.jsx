  ...
  return (
    ...
    >
      <div className="relative rounded-xl bg-linear-to-br from-accent-50 to-brand-100 h-[180px] overflow-hidden touch-pan-y select-none">
        ...
        {!oos && (
          <button onClick={(e) => { stop(e); openQuickAdd(p); }} aria-label="Add to cart" className="absolute bottom-1.5 right-1.5 z-10 w-7 h-7 rounded-full bg-white/90 backdrop-blur-sm text-brand-600 flex items-center justify-center shadow-sm active:scale-90 transition"><ShoppingCart size={14} /></button>
        )}
      </div>
