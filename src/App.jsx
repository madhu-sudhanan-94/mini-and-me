...
function Shell() {
  ...
  // Soft ambient wash behind the profile / account section (mobile only — reset at the lg breakpoint).
  const washScreen = ["account", "profile", "security", "addresses", "orders"].includes(screen);

  return (
    <div className="min-h-screen bg-slate-300 lg:bg-slate-50 flex justify-center sm:py-6 lg:py-0 font-sans">
      <style>{`.no-scrollbar::-webkit-scrollbar{display:none}.no-scrollbar{-ms-overflow-style:none;scrollbar-width:none}@keyframes vkUp{from{transform:translateY(100%);opacity:.7}to{transform:translateY(0);opacity:1}}.screen-wash{background:${profileWash}}@media(min-width:900px){.screen-wash{background:none}}`}</style>
