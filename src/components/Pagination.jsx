...
export default function Pagination({ page, pageCount, onChange }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 mt-6">
      <button onClick={() => go(page - 1)} disabled={page === 1} aria-label="Previous page" className={`${base} border border-slate-200 text-slate-600 disabled:opacity-50`}><ChevronLeft size={17} /></button>
      {pages.map((p, i) => p === "…"
      ...
      <button onClick={() => go(page + 1)} disabled={page === pageCount} aria-label="Next page" className={`${base} border border-slate-200 text-slate-600 disabled:opacity-50`}><ChevronRight size={17} /></button>
    </div>
  );
}
